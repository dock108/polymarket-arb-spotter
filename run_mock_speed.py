...
def run_alert_test(generator: MockDataGenerator, detector: ArbitrageDetector, 
                   target_alerts: int, duration_seconds: int, log_db_path: str) -> dict:
    """
    ...
    while stats['alerts_triggered'] < target_alerts and time.time() < end_time:
        ...
        for opp in opportunities:
            ...
            market = next(
                (m for m in batch if m['id'] == opp.market_id),
                None
            )
            ...
            if stats['alerts_triggered'] % 10 == 0:
                ...
                print(f"  Alerts: {stats['alerts_triggered']}/{target_alerts} "
                      f"({rate:.2f}/sec)")
    
    stats['end_time'] = datetime.now()
    duration = (stats['end_time'] - stats['start_time']).total_seconds()
    stats['duration_seconds'] = duration
    stats['markets_per_second'] = stats['markets_analyzed'] / duration if duration > 0 else 0
    stats['opportunities_per_second'] = stats['opportunities_found'] / duration if duration > 0 else 0
    stats['alerts_per_second'] = stats['alerts_triggered'] / duration if duration > 0 else 0
...
def main():
    ...
    if args.export_snapshots:
        ...
        export_path = generator.export_snapshots(
            count=args.num_markets,
            filepath=args.export_snapshots
        )
    ...
    try:
        ...
        if args.mode == 'alert-test':
            ...
            if alerts >= args.target_alerts:
                ...
                print(f"WARNING: Only generated {alerts}/{args.target_alerts} alerts "
