...
def run_analysis():
    ...
    if summary.pattern_stats:
        ...
        for pattern_type, stats in summary.pattern_stats.items():
            ...
            print(f"  Average price move: {stats.avg_price_move:.4f} ({stats.avg_price_move*100:.2f}%)")
            print(f"  Positive outcome rate: {stats.positive_outcome_rate:.2%}")
            print(f"  False positive rate: {stats.false_positive_rate:.2%}")
            print(f"  Average time to resolution: {stats.avg_time_to_resolution_minutes:.1f} minutes")
    ...
    if summary.signal_outcomes:
        ...
        for outcome in summary.signal_outcomes[:5]:  # Show first 5
            ...
            print(f"  Price after 5m: {outcome.price_after_5m:.4f}" if outcome.price_after_5m else "  Price after 5m: N/A")
            print(f"  Price after 15m: {outcome.price_after_15m:.4f}" if outcome.price_after_15m else "  Price after 15m: N/A")
            print(f"  Max price move: {outcome.max_price_move:.4f} ({outcome.max_price_move*100:.2f}%)")
            print(f"  Time to resolution: {outcome.time_to_resolution_minutes:.1f}m" if outcome.time_to_resolution_minutes else "  Time to resolution: N/A")
