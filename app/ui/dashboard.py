...
from app.core.logger import logger, fetch_recent
...
def render_dashboard():
    ...
    """
    st.set_page_config(
        page_title="Polymarket Arbitrage Spotter",
        page_icon="🎯",
        layout="wide"
    )
...
def render_dashboard_content():
    ...
    if recent_opps:
        ...
        display_cols = ['detected_at', 'market_name', 'opportunity_type', 
                       'expected_profit', 'expected_return_pct', 'risk_score']
        df_display = df[display_cols].copy()
        df_display.columns = ['Time', 'Market', 'Type', 'Profit ($)', 'Return (%)', 'Risk']
        ...
        st.info("No opportunities detected yet. Generate some data using the controls below.")
    ...
    with summary_col1:
        ...
        profitable_count = sum(1 for event in recent_events if event.get('decision') == 'alerted')
        st.metric("Profitable Detected", profitable_count)
    
    with summary_col2:
        if recent_events:
            avg_profit_pct = sum(event.get('expected_profit_pct', 0) for event in recent_events) / len(recent_events)
...
def generate_mock_data():
    ...
    logger.info(f"Generated {len(opportunities)} opportunities from {len(markets)} markets")
...
def render_history_view():
    ...
    if decision_filter != "All":
        events = [e for e in events if e.get('decision', '').lower() == decision_filter.lower()]
    ...
    with stat_col3:
        if events:
            avg_profit = sum(e.get('expected_profit_pct', 0) for e in events) / len(events)
    ...
    if events:
        ...
        display_cols = ['timestamp', 'market_name', 'expected_profit_pct', 
                       'decision', 'mode', 'failure_reason']
...
def render_settings_view():
    ...
    st.info("📖 Settings are currently read-only. Configure via environment variables or .env file.")
